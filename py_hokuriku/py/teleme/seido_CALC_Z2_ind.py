...
#--------------------
def calc_mm_hh_PV(df,col,h_shift=1,m_shift=1,cate2="Z2"): #2021.10.03
  """""
  ...
  for hh in _hh:
    ...
    a,b,c = load_mm_hh_abc2(mm,hh,cate=cate,cate2=cate2) #2021.12.08
    # print(mm,hh,a,b,c)
    # sys.exit()
    if a !=9999:
...
def calc_hh_PV(df,col,cate2): #2021.10.03
  """""
  時刻別の推定値を計算するsubroutine　#2021.10.03
  #2021.11.19
  Args:
  ...
  for hh in _hh:
    ...
    a,b,c = load_hh_abc2(hh,cate2) #2021.12.08 sorimachi making !!!!
    if a !=9999:
...
def calc_mm_PV(df,mm,cate2):
  ...
  for i in range(3):
    ...
    if i==1:
      a,b,c = get_a_b_c("201905",cate="obs",with_smame=True)
      hosei_ratio = get_hosei(mm,cate2)
    if i==2:
...
def seido_mm(radname="8now0",p=90,cate="train",cate2="Z2"):
  ...
  for i ,mm in enumerate(_mm):
    ...
    # -------------TELEME 1 ---------
    df0 = load_PV_per_cate(mm,cate2="T1")
    # print(df0.describe())
    df0 = calc_mm_PV(df0,mm,cate2="T1")
    df0 = calc_hh_PV(df0,col="PV-max[3]",cate2="T1")
    df0 = calc_mm_hh_PV(df0,col="PV-max[4]",cate2="T1")
    df0.columns = [f"{c}_T1" for c in df0.columns ]
    # -------------SMAME(1) All ---------
    df1 = load_PV_per_cate(mm,cate2="Z1")
    # print(df1.describe())
    df1 = calc_mm_PV(df1,mm,cate2="Z1")
    df1 = calc_hh_PV(df1,col="PV-max[3]",cate2="Z1")
    df1 = calc_mm_hh_PV(df1,col="PV-max[4]",cate2="Z1")
    df1.columns = [f"{c}_Z1" for c in df1.columns ]
    df = pd.concat([df0,df1],axis=1)
    #-----------------------
    # print(df1.describe())
    # sys.exit()
    
    df["sum"] = df[["sum_T1","sum_Z1"]].sum(axis=1)
    df["PV-max[0]"] = df[["PV-max[0]_T1","PV-max[0]_Z1"]].sum(axis=1)
    df["PV-max[1]"] = df[["PV-max[1]_T1","PV-max[1]_Z1"]].sum(axis=1)
    df["PV-max[2]"] = df[["PV-max[2]_T1","PV-max[2]_Z1"]].sum(axis=1)
    df["PV-max[3]"] = df[["PV-max[3]_T1","PV-max[3]_Z1"]].sum(axis=1)
    df["PV-max[4]"] = df[["PV-max[4]_T1","PV-max[4]_Z1"]].sum(axis=1)
    
    use_col = ["sum","PV-max[0]","PV-max[1]","PV-max[2]","PV-max[3]","PV-max[4]"]
    df = df[use_col]
  ...
  df.index.name="month"
  df.columns = _col
  df.to_csv(f"{ERR}/seido_mm_{cate}_teleme_{radname}_ind.csv")
  #---------------------------------------------------------
  
  df = pd.concat(_df_aa,axis=0)
  estimate_path = f"{ESTIMATE}/PV_{cate}_teleme_{radname}_ind.csv"
  df.to_csv(estimate_path)
  print(datetime.now() , "[END]",estimate_path)
  return

def seido_hh(radname="8now0",cate="train"):
  """
  2021.09.20 : 時刻別評価
  2021.09.20 : 時刻別 -> selectで調整したデータ
  2021.12.08 : 時刻別/設備容量別の予測データ
  """
  path = f"{ESTIMATE}/PV_{cate}_teleme_{radname}_ind.csv"
  df = pd.read_csv(path)
  #-------------
  df = df.reset_index()
  ...
  df.columns = _col
  df = df.replace(9999,np.nan)
  df.to_csv(f"{ERR}/seido_hh_{cate}_teleme_{radname}_ind.csv")
  print(datetime.now() , "[END]",f"{ERR}/seido_hh_{cate}_teleme_{radname}_ind.csv")
  return
...
if __name__ == "__main__":
  ...
  if 1:
    ...
    # for cate in ["test"]:
      seido_mm(radname=radname,cate=cate) # 月別 21.11.19 mesh-PVも()
      seido_hh(radname=radname,cate=cate) # 時刻別 21.11.19
      # sys.exit()
