...
def ts_TELEME_dd(dd,plot="overfit"):
  ...
  for i,c in enumerate(_c):
    ...
    ax.plot(np.arange(len(df)), df[c], label=label,lw=lw)
  ...
  f.savefig(f"{OUTD}/pv_{dd}.png", bbox_inches="tight")
  plt.close()
  print(datetime.now(),"[END]",f"{OUTD}/pv_{dd}.png")
  return
...
if __name__ == "__main__":
  ...
  if 1:
    ...
    # _c1 = ['PV-max[2]','PV-max[3]', 'PV-max[4]', 'PV-max_M1_SN0', 'PV-max_M2_SN0','PV-max_M1_SN1', 'PV-max_M2_SN1']
    #-------------
    # _dd = select_dd(cate="test",GOOD=False,c1 ='PV-max[4]')[:10]
    plot_type = "overfit"
    ...
    # sys.exit()
    for dd in _dd:
      ts_TELEME_dd(dd=dd, plot=plot_type)
      # points_TELEME_dd(dd=dd)
      # code_TELEME_dd(dd=dd, code="telm006")
      # sys.exit()