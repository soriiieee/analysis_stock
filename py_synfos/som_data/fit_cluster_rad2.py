...
ESTIMATE3="/home/ysorimachi/data/synfos/som/model/estimate3" #保存先
...
MODEL1="/home/ysorimachi/data/synfos/som/model/m1"
MODEL2="/home/ysorimachi/data/synfos/som/model/m2"
MODEL3="/home/ysorimachi/data/synfos/som/model/m3"
...
def train_reg(cele="MSPP",ecode="ecmf001",resid2_name="lgb",n_dim=3,isCNN=1):
    ...
    for lbl in range(n_dim**2):
        ...
        if n !=0 :
            ...
            #---pred1(fit)---#
            """"[summary]"
            ここからが、全気象要素を盛り込んだ統合回帰　ver　
            2021.11.19
            """
            x_col = ["SYN","EC","CR0"] + use_reg_weather_element()
            y_col = "OBS"
            ...
            if resid2_name =="lr":
                ...
                model = Resid2(name=resid2_name,pretrained=False)
                model.fit(X,y)
            model_path = f"{MODEL3}/{resid2_name}_{ecode}_{cele}_cls{lbl}.pkl"
            
            save_model(model_path,model) #save
...
def predict_rad(cele="MSPP",ecode="ecmf001",resid2_name="lgb",n_dim=3,isCNN=1):
    ...
    """
    # FIT_DIR="/home/ysorimachi/data/synfos/som/model/fitting_synfos"
    def predict_calc(df,model):
        # x_col = ["SYN","EC","CR0"]
        # x2_col = ["70RH","85RH","70UU","70VV","LOCA","MICA"]
        # _cele = ["70RH","85RH","70UU","70VV","LOCA","MICA"] #2021.10.15
        x2_col = ["SYN","EC","CR0"] + use_reg_weather_element()
        # y_col = "OBS"
        X = df[x2_col]
        
        col_name = f"PRED_{resid2_name}"
        df[col_name] = df["CR0"] #初期値
        
        def clensing1(x):
            if x[col_name]<0:
                return 0
            elif x[col_name] > x["CR0"]:
                return x["CR0"]
            else:
                return x[col_name]
        # print(X.head())
        ...
        df[col_name] = df.apply(clensing1,axis=1)
    ...
    for lbl in tqdm(list(range(n_dim**2))):
        ...
        if n:
            ...
            # print(df)
            # sys.exit()
            model = load_model(f"{MODEL3}/{resid2_name}_{ecode}_{cele}_cls{lbl}.pkl")
    ...
    df.to_csv(f"{ESTIMATE3}/rad_{resid2_name}_{ecode}_{cele}_{n_dim}_{isCNN}.csv", index=False)
...
if __name__== "__main__":
    ...
    if 1:
        ...
        _ecode = ["ecmf003"] #東京のみ
        _cele = ["MSPP","HICA"]
        _resid2 = ["lr","lasso","ridge","svr","tree","rf","lgb","mlp3","mlp5"]
        # _resid2 = ["mlp3","mlp5"]
        # -----------------------------------------
        # Debug --- ----------
        # _cele = ["MSPP"]
