...
def train_SOM_cluster(cele="sp",n_month=12,n_dim=5,isCNN=1,som_model="SOM-ED"):
  """[summary]
  2021.10.10 init
  2021.11.18 update
  必要事項を記入して、somのクラスタリングを実施する流れを実装する

  Args:
      cele (str, optional): [description]. Defaults to "sp".
      n_month (int, optional): [description]. Defaults to 12.
      n_dim (int, optional): [description]. Defaults to 5.
      _isCNN (int, optional): [description]. Defaults to 1.
      som_model (str, optional): [description]. Defaults to "SOM-ED".
  Return:
      None
  """
  nc = OpenNetCDF()
  _mm = loop_mm(st=2001,ed=2020)[:n_month] #201001~201912
...
# loop 
  ...
  #som trainer ---
  print("som_model->", som_model ,"MAP-dimention->",n_dim)
  cm = ClusterModel(som_model,n_clusters=n_dim*n_dim,DIMENTION=DIMENTION,max_iter=100000,epoch=1)
  cm.fit(img2)
  
  # som model save ---
  cm_path = f"{CLUSTER}/{cele}_{n_dim}_{isCNN}.pkl"
  save_model(cm_path,cm)
  # som label save ---
  df = pd.DataFrame()
  df["time"] = _time
  df["label"] = cm.labels_
  df.to_csv(f"{LABELS}/{cele}_{n_dim}_{isCNN}.csv", index=False)
  # som mesh save ---
  f,ax = plt.subplots(n_dim,n_dim,figsize=(25,25))
  ax = ax.flatten()
  ...
  f.savefig(f"{MESH}/{cele}_{n_dim}_{isCNN}.png", bbox_inches="tight")
  plt.close()
...
def load_cnn():
  ...
  path = "/home/ysorimachi/data/synfos/som/cnn/2D56_to_1D128.pkl"
...
def som_dist_mm(cele="sp",n_dim=3,isCNN=1):
  """[summary]
  2021.10.13
  2021.11.18
  som のクラスタリングで分類された月別の頻度を表示する
  """
  # from a01_99_utils import *
  # cele="HICA"
  # cele="MSPP"
  data = np.zeros((12,n_dim**2))
  path = f"/home/ysorimachi/data/synfos/som/model/labels/{cele}_{n_dim}_{isCNN}.csv"
  df = pd.read_csv(path)
  ...
  for i,lbl in enumerate(list(range(n_dim**2))):
    # n,_dd = select_som_day(cele,lbl=lbl,istrain=False,istest=False)
    _dd = df[df["label"]==lbl]["dd"].values.tolist()
    n = len(_dd)
    # print(_dd)
    # sys.exit()
    if n !=0:
  ...
  df = pd.DataFrame(data,index=range(1,12+1), columns = [f"cls{str(i).zfill(2)}" for i in range(n_dim**2)])
  for c in df.columns:
    df[c] = df[c].astype(int)
  df.index.name = "mm"
  f,ax = plt.subplots(figsize=(15,6))
  sns.heatmap(df, annot=True,square=False, fmt='d',ax=ax, cmap="Oranges")
  f.savefig(f"/home/ysorimachi/data/synfos/som/png/heatmap_{cele}_{n_dim}_{isCNN}.png", bbox_inches="tight")
  print(f"/home/ysorimachi/data/synfos/som/png/heatmap_{cele}_{n_dim}_{isCNN}.png")
  return
...
if __name__== "__main__":
  ...
  if 1:
    ...
    #cleanier ------------------
    # sys.exit()
    n_month=12*18 #2010 年から10年
    som_model = "SOM-ED" # "SOM-ED" "SOM-SSIM"
    _n_dim =[3,5] #som map size
    _isCNN =[1]
    # _cele = ["sp"] # "sp","mc","lc","hc"
    _cele = ["sp","hc"] # "sp","mc","lc","hc"
    
    for isCNN in _isCNN:
      for n_dim in _n_dim:
        for cele in _cele: #"sp","mc","lc","hc"
          # train_SOM_cluster(cele=cele,n_month=n_month,n_dim=n_dim,isCNN=isCNN,som_model=som_model)
          som_dist_mm(cele=cele,n_dim=n_dim,isCNN=isCNN) #2021.10.05
          # sys.exit()
